...
## defaults
rawDataFile=${PWD}/data/batch_1.json
...
numHudiPartitions=5
partitionField=date
schemaFile=${PWD}/schema/schema.avsc
numBatch=1
recordValue=0
recreateTopic="Y"
...
if [ $recreateTopic = "Y" ]; then
  ...
  docker-compose exec broker kafka-topics --delete --topic ${kafkaTopicName} --bootstrap-server ${kafkaBrokerHostname}:9092

  # Create the topic with 4 partitions
  echo "Create Kafka topic $kafkaTopicName ..."
  docker-compose exec broker kafka-topics --create --topic ${kafkaTopicName} --partitions $numKafkaPartitions --replication-factor 1 --bootstrap-server ${kafkaBrokerHostname}:9092
fi
...
for ((i = 1;i<=numBatch;i++)); do
  ...
  grep -v '^$' ${events_file} | kafkacat -P -b ${kafkaBrokerHostname}:9092 -t ${kafkaTopicName}
done
